...
//why not use integers >1,-1 e.g. for packing purposes
void ToolCalc::xyz2grid(int na, int nb, int nc, bool verbose) {
	gx = na;
	gy = nb;
	gz = nc;
	gridp = na * nb * nc;
			...
			for (int c = 0; c < nc; c++) {
				...
				for (int i = 0; i < atnumber * 3; i += 3) {
					...
					if (((*xyz)(i) > cubex) && ((*xyz)(i) < (cubex + spacex))
							&& (((*xyz)(i + 1) > cubey) && ((*xyz)(i + 1)
									< (cubey + spacey))) && (((*xyz)(i + 2)
							> cubez) && ((*xyz)(i + 2) < (cubez + spacez)))) {
						//cout << "Hit:"<< (*xyz)(i) << " " <<(*xyz)(i+1) << " "
						//		<<(*xyz)(i+2)<< endl;
						//cout << "Actual seg: "<< segment<< " coords x:" <<cubex
						//	<<" y:"<<cubey <<" z:" <<cubez<<endl;
						if (segment == segment_old) {
							cout << "\nWARNING: Two atoms at one grid_point:" << segment <<endl;
							segment++;
						}
...
void ToolCalc::grid2genom(bool verbose) {
	ToolIO newIO;
	...
	if (gridp_compressed != atnumber ) {
			cout << "\ngrid2genom: something went terribly wrong: number of atoms:"<<atnumber<<" does not coincide with genes:"<< gridp_compressed<<" Exiting..." << endl;
...
void ToolCalc::grid2xyz(bool verbose) {
	double cubex = 0, cubey = 0, cubez = 0;
	double spacex = 0, spacey = 0, spacez = 0;
	int segment = 0;
	int pos = 0;
